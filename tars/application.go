...
import (
	...
	"github.com/MacgradyHuang/TarsGo/tars/protocol/res/adminf"
	"github.com/MacgradyHuang/TarsGo/tars/transport"
	"github.com/MacgradyHuang/TarsGo/tars/util/conf"
...
//// zaplog is the logger for tars framework.
//var zaplog = rogger.GetLogger("zaplog")
...
func initConfig() {
	...
	if len(ServerConfigPath) == 0 {
		zaplog.Error("server config path empty")
		return
	}

	c, err := conf.NewConf(ServerConfigPath)
	if err != nil {
		zaplog.Error("open server config fail")
		return
	}
	...
	if svrCfg != nil {
		logPath := svrCfg.LogPath + "/" + svrCfg.App + "/" + svrCfg.Server + "/" + svrCfg.Server + ".log"
		zaplog.InitZapLogger(zaplog.ProcessName(path.Base(os.Args[0])+"_rd"), zaplog.LogPath(logPath))
	}
	...
	zaplog.Debug("config add", zap.Any("Config", tarsConfig))
...
func Run() {
	...
	for _, env := range os.Environ() {
		if strings.HasPrefix(env, grace.InheritFdPrefix) {
			zaplog.Info("env", zap.String("Env", env))
		}
	}
	...
	for _, obj := range objRunList {
		if s, ok := httpSvrs[obj]; ok {
			...
			go func(obj string) {
				addr := s.Addr
				zaplog.Info("http server start", zap.String("Obj", obj), zap.String("Addr", s.Addr))
				if addr == "" {
					lisDone.Done()
				...
				if err != nil {
					if err == http.ErrServerClosed {
						zaplog.Info("http server stop", zap.String("Obj", obj), zap.Error(err))
					} else {
		...
		zaplog.Debug("Run", zap.String("Obj", obj), zap.Any("Config", s.GetConfig()))
		lisDone.Add(1)
		go func(obj string) {
	...
	if os.Getenv("GRACE_RESTART") == "1" {
		ppid := os.Getppid()
		zaplog.Info("stop ppid", zap.Int("Ppid", ppid))
		if ppid > 1 {
...
func graceRestart() {
	pid := os.Getpid()
	zaplog.Debug("grace restart server begin", zap.Int("Pid", pid))
	...
	if cfg != nil {
		...
		zaplog.Debug("redirect to", zap.String("LogPath", logpath), zap.Any("LogFile", logfile))
	}
	if logfile == nil {
		logfile = os.Stdout
	}
	...
	for key, file := range grace.GetAllLisenFiles() {
		...
		newFd := len(files)
		zaplog.Debug("translate", zap.String(key, fd), zap.Int(key, newFd))
	...
	if err != nil {
		zaplog.Error("LookPath failed", zap.Error(err))
		return
	}
	...
	if err != nil {
		zaplog.Error("start supprocess failed", zap.Error(err))
		return
	}
	zaplog.Info("subprocess start", zap.Int("Pid", process.Pid))
	go process.Wait()
}
...
func graceShutdown() {
	...
	zaplog.Info("grace shutdown start", zap.Int("Pid", pid), zap.Any("timeout", graceShutdownTimeout))
	...
	for _, obj := range destroyableObjs {
		...
		go func(wg *sync.WaitGroup, obj destroyableImp) {
			defer wg.Done()
			obj.Destroy()
			zaplog.Info("grace Destroy success", zap.Int("Pid", pid))
		}(&wg, obj)
	}
	...
	for _, obj := range objRunList {
		if s, ok := httpSvrs[obj]; ok {
			...
			go func(s *http.Server, ctx context.Context, wg *sync.WaitGroup, objstr string) {
				...
				err := s.Shutdown(ctx)
				if err == nil {
					zaplog.Info("grace shutdown http success", zap.String("Obj", objstr), zap.Int("Pid", pid))
				} else {
					zaplog.Info("grace shutdown http failed", zap.String("Obj", objstr), zap.Any("timeout", graceShutdownTimeout), zap.Error(err))
		...
		if s, ok := goSvrs[obj]; ok {
			...
			go func(s *transport.TarsServer, ctx context.Context, wg *sync.WaitGroup, objstr string) {
				...
				err := s.Shutdown(ctx)
				if err == nil {
					zaplog.Info("grace shutdown tars success", zap.String("Obj", objstr), zap.Int("Pid", pid))
				} else {
					zaplog.Info("grace shutdown tars failed", zap.String("Obj", objstr), zap.Any("timeout", graceShutdownTimeout), zap.Error(err))
				}
			}(s, ctx, &wg, obj)
		}
	}
	...
	select {
	case <-ctx.Done():
		zaplog.Info("grace shutdown all success", zap.Any("timeout", graceShutdownTimeout))
	case <-time.After(graceShutdownTimeout):
		zaplog.Info("grace shutdown timeout", zap.Any("timeout", graceShutdownTimeout))
	}
...
func teerDown(err error) {
	shutdownOnce.Do(func() {
		if err != nil {
			...
			fmt.Println(err)
			zaplog.Error("teerDown err", zap.Error(err))
		}
		shutdown <- true
	})
}
