...
import (
	...
	"github.com/MacgradyHuang/TarsGo/tars/protocol/res/queryf"
	"github.com/MacgradyHuang/TarsGo/tars/util/consistenthash"
	"github.com/MacgradyHuang/TarsGo/tars/util/endpoint"
	"github.com/MacgradyHuang/TarsGo/tars/util/gtime"
)
...
func GetManager(comm *Communicator, objName string) EndpointManager {
	...
	zaplog.Debug("Create endpoint manager for ", zap.String("Obj", objName))
	...
	if err != nil {
		for _, cache := range appCache.ObjCaches {
			if objName == cache.Name && comm.GetLocator() == cache.Locator {
				...
				zaplog.Debug("init endpoint", zap.String("Obj", objName), zap.Any("Active", em.activeEp), zap.Any("Inactive", em.inactiveEpf))
			}
		}
	}
	g.mlock.Unlock()
...
func (g *globalManager) updateEndpoints() {
	...
	for range loop.C {
		...
		g.mlock.Unlock()
		zaplog.Debug("start refresh endpoints", zap.Int("EndpointLen", len(eps)), zap.Int("RefreshInterval", g.refreshInterval))
		for _, e := range eps {
			err := e.doFresh()
			if err != nil {
				zaplog.Error("update endpoint error", zap.String("Obj", e.objName))
			}
...
func newTarsEndpointManager(objName string, comm *Communicator) *tarsEndpointManager {
	...
	if pos > 0 {
		...
		zaplog.Debug("proxy mode:", zap.String("Obj", objName))
		e.objName = objName
		e.directproxy = false
		obj, _ := e.comm.GetProperty("locator")
		e.locator = new(queryf.QueryF)
		zaplog.Debug("string to proxy locator", zap.String("Obj", obj))
		...
		if v, ok := e.epList.Load(ep.Key); ok {
			...
			if needCheck {
				if _, ok := e.checkAdapterList.Load(ep.Key); !ok {
					...
					e.checkAdapter <- adp
					zaplog.Error("checkStatus|insert check adapter", zap.Any("Endpoint", ep.Key))
				}
			}
		}
	}
...
func (e *tarsEndpointManager) SelectAdapterProxy(msg *Message) (*AdapterProxy, bool) {
	...
	case adp := <-e.checkAdapter:
		zaplog.Error("SelectAdapterProxy|check adapter", zap.Any("Endpoint", adp.GetPoint()))
...
func (e *tarsEndpointManager) findAndSetObj(q *queryf.QueryF) error {
	...
	if err != nil {
		zaplog.Error("findAndSetObj fail", zap.String("Obj", e.objName), zap.Error(err))
		return err
	}
	if ret != 0 {
		e := fmt.Errorf("findAndSetObj %s fail, ret: %d", e.objName, ret)
		zaplog.Error(e.Error())
		return e
	}
	...
	/*
		if endpoint.IsEqaul(activeEp, &e.activeEpf) {
			zaplog.Debug("endpoint not change: ", e.objName)
			return nil
		}
	*/
	if len(activeEp) == 0 {
		zaplog.Error("findAndSetObj, empty of active endpoint", zap.String("Obj", e.objName))
		return nil
	}
	zaplog.Debug("findAndSetObj|call FindObjectById ok", zap.String("Obj", e.objName), zap.Int32("Ret", ret), zap.Any("Active", activeEp), zap.Any("Inactive", inactiveEp))
	...
	e.epList.Range(func(key, value interface{}) bool {
		...
		if !flagActive && !flagInactive {
			...
			e.epList.Delete(key)
			zaplog.Debug("findAndSetObj|delete useless endpoint from epList", zap.Any("Key", key))
		}
		return true
	})
	...
	zaplog.Debug("findAndSetObj|activeEp", zap.Any("SortedEps", sortedEps))
	return nil
}
