...
func (c *connection) send(conn net.Conn, connDone chan bool) {
	...
	for {
		...
		if err != nil {
			//TODO add retry time
			c.tc.sendQueue <- req
			zaplog.Error("send request error:", zap.Error(err))
			c.close(conn)
			return
		}
	}
	...
	for {
		...
		if err != nil {
			...
			if _, ok := err.(*net.OpError); ok {
				zaplog.Error("net.OpError", zap.Any("RemoteAddr", conn.RemoteAddr()), zap.Error(err))
				c.close(conn)
				return // connection is closed
			}
			if err == io.EOF {
				zaplog.Debug("connection closed by remote", zap.Any("RemoteAddr", conn.RemoteAddr()), zap.Error(err))
			} else {
				zaplog.Error("read package error:", zap.Error(err))
			}
			c.close(conn)
			return
		}
		...
		for {
			...
			zaplog.Error("parse package error")
			c.close(conn)
			return
		}
	}
...
func (c *connection) ReConnect() (err error) {
	c.connLock.Lock()
	if c.isClosed {
		zaplog.Debug("Connect:", zap.String("Address", c.tc.address))
...
func (c *TarsClient) GraceClose(ctx context.Context) {
	...
	for {
		...
		case <-ctx.Done():
			return
		case <-tk.C:
			zaplog.Debug("wait grace invoke", zap.Int32("InvokeNum", c.conn.invokeNum))
