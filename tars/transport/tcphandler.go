...
import (
	...
	"github.com/MacgradyHuang/TarsGo/tars/util/gpool"
...
func (h *tcpHandler) Listen() (err error) {
	...
	if err == nil {
		zaplog.Info("Listening", zap.String("Address", cfg.Address))
		h.lis = ln.(*net.TCPListener)
	} else {
		zaplog.Info("Listening error", zap.String("Address", cfg.Address), zap.Error(err))
	}
...
func (h *tcpHandler) handleConn(connSt *connInfo, pkg []byte) {
	handler := func() {
		...
		if _, err := connSt.conn.Write(rsp); err != nil {
			zaplog.Error("send pkg failed", zap.Any("RemoteAddr", connSt.conn.RemoteAddr()), zap.Error(err))
		}
...
func (h *tcpHandler) Handle() error {
	...
	for {
		if atomic.LoadInt32(&h.ts.isClosed) == 1 {
			zaplog.Error("Close accept", zap.String("Address", h.conf.Address), zap.Int("Pid", os.Getpid()))
		...
		if err != nil {
			if !isNoDataError(err) {
				zaplog.Error("Accept error:", zap.Error(err))
			} else if conn != nil {
		...
		go func(conn *net.TCPConn) {
			...
			zaplog.Debug("TCP accept:", zap.Any("RemoteAddr", conn.RemoteAddr()), zap.Int("Pid", os.Getpid()), zap.String("Key", key))
...
func (h *tcpHandler) sendCloseMsg() {
	...
	h.conns.Range(func(key, val interface{}) bool {
		...
		zaplog.Debug("send close message", zap.Any("RemoteAddr", conn.conn.RemoteAddr()))
		conn.writeLock.Lock()
...
func (h *tcpHandler) CloseIdles(n int64) bool {
	if atomic.LoadInt32(&h.isListenClosed) == 0 {
		...
		zaplog.Debug("Hack msg", zap.String("Address", h.conf.Address))
	...
	h.conns.Range(func(key, val interface{}) bool {
		conn := val.(*connInfo)
		zaplog.Debug("num invoke", zap.Int32("NumInvoke", atomic.LoadInt32(&conn.numInvoke)), zap.Bool("Idle", conn.idleTime+n > time.Now().Unix()))
...
func (h *tcpHandler) recv(connSt *connInfo) {
	...
	defer func() {
		...
		zaplog.Debug("Close connection", zap.Any("RemoteAddr", conn.RemoteAddr()))
		conn.Close()
		connSt.idleTime = 0
	}()
	...
	for {
		...
		// zaplog.Debugf("%s closed: %d, read %d, nil buff: %d, err: %v", h.ts.conf.Address, atomic.LoadInt32(&h.ts.isClosed), n, len(currBuffer), err)
		if err != nil {
			...
			if err == io.EOF {
				zaplog.Debug("connection closed by remote:", zap.Any("RemoteAddr", conn.RemoteAddr()))
			} else {
				zaplog.Error("read package error:", zap.Any("Type", reflect.TypeOf(err)), zap.Error(err))
			}
			return
		}
		...
		for {
			...
			zaplog.Errorf("parse package error %s %v", conn.RemoteAddr(), err)
			return
		}
	}
}
